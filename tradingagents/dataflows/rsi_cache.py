...
rsi_cache.py
------------
Persistent SQLite cache for optimized RSI parameters per ticker.

Each ticker stores one row — the latest optimization result.
On cache hit the graph skips the optimizer entirely, saving ~2-3 sec per run.
Force a refresh by calling upsert_rsi_params() directly or setting
force_reoptimize=True in the optimizer node.

Table: rsi_params_cache
"""
...
# Dedicated RSI cache DB — lives in data/ at project root.
...
def _get_conn() -> sqlite3.Connection:
    """
    Open a connection to the RSI cache SQLite DB.
    ...
    The path is percent-encoded before embedding in the URI so spaces and
    other special characters in the project path do not break the URI.
    """
...
def _ensure_table() -> None:
    """Create rsi_params_cache table if it doesn't exist (idempotent)."""
    conn = _get_conn()
    conn.execute("""
...
def get_rsi_params(ticker: str) -> Optional[Dict[str, Any]]:
    """
    Load cached RSI params for a ticker.
    ...
    conn = _get_conn()
    row = conn.execute(
        "SELECT * FROM rsi_params_cache WHERE ticker = ?",
        (ticker.upper().strip(),)
    ).fetchone()
    conn.close()
...
def upsert_rsi_params(
    ...
    """
    Insert or replace RSI optimization result for a ticker.
    optimized_at is automatically set to current UTC timestamp.
    """
    conn = _get_conn()
    conn.execute("""
        INSERT INTO rsi_params_cache
            (ticker, optimal_period, optimal_upper, optimal_lower,
             oos_sharpe, is_sharpe, confidence, regime,
        ...
        ON CONFLICT(ticker) DO UPDATE SET
            ...
            optimal_upper      = excluded.optimal_upper,
            optimal_lower      = excluded.optimal_lower,
...
def list_all_cached_tickers() -> list:
    """Return all tickers that have cached RSI params, with their metadata."""
    conn = _get_conn()
    rows = conn.execute("""
        SELECT ticker, optimal_period, optimal_upper, optimal_lower,
               oos_sharpe, confidence, regime, optimizer_provider, optimized_at
        FROM rsi_params_cache
        ORDER BY ticker ASC
    """).fetchall()
    conn.close()
...
def delete_rsi_params(ticker: str) -> bool:
    """Remove cached params for a ticker. Returns True if a row was deleted."""
    conn = _get_conn()
    cursor = conn.execute(
        "DELETE FROM rsi_params_cache WHERE ticker = ?",
        (ticker.upper().strip(),)
    )
    conn.commit()
